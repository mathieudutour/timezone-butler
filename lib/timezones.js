...
// match 1 EST
const shortTimezone = `(\\d{1,2})\\s?${timezonesRegex}`
...
const REGEX = new RegExp(
  `${timezonesRegex}?\\s?(${longTime}|${shortTime}|${shortTimezone})\\s?${timezonesRegex}?`,
  'gi'
)
...
module.exports = message => {
  ...
  while (match) {
    ...
    const timezoneModifier = match[1] || match[11] || match[10]
