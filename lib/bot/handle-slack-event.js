...
const handleSlackUserChanged = async (teams, body, event, updateUser) => {
  if (!event.user) {
    return 'missing user from event'
  }
  if (!event.user.id) {
    const user = await slackRequest.getUserInfo(
      teams[body.team_id]._token,
      event.user
    )
  ...
  updateUser(body.team_id, event.user.id, parseFloat(event.user.tz_offset))
...
module.exports = (teams, body, res, updateUser) => {
  ...
  if (event.type === 'user_change') {
    return handleSlackUserChanged(teams, body, event, updateUser)
  }
